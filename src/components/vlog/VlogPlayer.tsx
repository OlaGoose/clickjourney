...
const ANIMATION_VARIANTS = [
  {
    ...
    transition: {
      ...
      scale: { duration: SLIDE_DURATION + 2, ease: 'linear' as const },
    },
  },
  {
    initial: { opacity: 0, x: '15%', scale: 1.1 },
    animate: { opacity: 1, x: '0%', scale: 1.0 },
    exit: { opacity: 0, x: '-5%', scale: 1.0 },
    transition: {
      opacity: { duration: TRANSITION_DURATION, ease: 'easeInOut' as const },
      x: { duration: SLIDE_DURATION, ease: [0.22, 1, 0.36, 1] as const },
      scale: { duration: SLIDE_DURATION, ease: [0.22, 1, 0.36, 1] as const },
    },
  },
  {
    ...
    transition: {
      opacity: { duration: TRANSITION_DURATION, ease: 'easeInOut' as const },
      y: { duration: SLIDE_DURATION + 1, ease: [0.25, 0.46, 0.45, 0.94] as const },
  ...
  },
  {
    ...
    transition: {
      opacity: { duration: TRANSITION_DURATION, ease: 'easeInOut' as const },
      filter: { duration: 3, ease: 'easeOut' as const },
      scale: { duration: SLIDE_DURATION + 2, ease: 'linear' as const },
    },
  },
];
...
export function VlogPlayer({ data, onExit }: VlogPlayerProps) {
  ...
  const getBackgroundImage = (item: PlaylistItem | undefined) => {
    if (!item) return '';
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-black select-none">
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center">
        <div className="relative w-full h-full md:w-[56.25vh] md:h-full overflow-hidden shadow-2xl bg-black">
          ...
          <div
            className={`absolute inset-0 pointer-events-none z-20 ${activeFilter.overlayColor}`}
          />
          <div
      ...
      {/* First frame: cinematic location title at top center */}
